...
<body class="h-full w-full m-0 p-0 flex items-center justify-center p-6 bg-[linear-gradient(rgba(0,0,0,0.5),rgba(0,0,0,0.5)),url('login_bg.jpg')] bg-cover bg-center bg-no-repeat bg-fixed font-['Poppins']">
    ...
    <form id="form1" runat="server" class="w-full max-w-md bg-white/95 backdrop-blur-md p-8 rounded-xl border border-zinc-200 shadow-xl space-y-6" onkeydown="if(event.keyCode==13) { return false; }">
        ...
        <header class="border-b border-[#F1F1EF] pb-4">
            ...
            <p class="text-[#7C7B77] text-sm mt-1">Lost your security credentials? Enter your system email to provisioning a temporary access key.</p>
        </header>
        ...
        <asp:Panel ID="pnlSuccessDetails" runat="server" CssClass="bg-emerald-50 border border-emerald-200 p-4 rounded-lg text-xs space-y-2 text-emerald-800" Visible="false">
            ...
            <div class="bg-white p-2.5 rounded border border-emerald-300 font-mono text-center text-sm font-bold tracking-wider select-all my-2">
        ...
        <div class="space-y-4">
            <div>
                ...
                <asp:TextBox ID="txtRecoveryEmail" runat="server" TextMode="Email" CssClass="w-full bg-[#F7F7F5]/50 p-2.5 text-sm rounded border border-[#EBEBE9] focus:border-[#1A1A1A] outline-none text-[#2F2F2F]" placeholder="e.g., student@inti.edu.my"></asp:TextBox>
            </div>
