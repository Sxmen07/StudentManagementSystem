    ...
    public partial class CreateAccounts : System.Web.UI.Page
    {
        ...
        protected void btnCreateAccount_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection conn = new SqlConnection(connString))
            {
                try
                {
                    ...
                    if (isUpdate)
                    {
                        ...
                        if (originalRole != role)
                        {
                            ...
                            string dropQuery = originalRole == "Admin" ? "DELETE FROM HeadofProgramme WHERE HopID = @ID" :
                                               originalRole == "Lecturer" ? "DELETE FROM Lecturer WHERE LecturerID = @ID" :
                            ...
                            string insertQuery;
                            if (role == "Admin")
                                insertQuery = "INSERT INTO HeadofProgramme (HopName, HopEmail, Password, IdentityNumber) VALUES (@Name, @Email, @Password, @IDNum)";
                            else if (role == "Lecturer")
                                insertQuery = "INSERT INTO Lecturer (LecturerName, LecturerEmail, Password, IdentityNumber) VALUES (@Name, @Email, @Password, @IDNum)";
                            else // Student
                                insertQuery = "INSERT INTO Student (StudentName, StudentEmail, Password, IC, IdentityNumber, SemesterID, ProgrammeCode, IntakeYear) VALUES (@Name, @Email, @Password, @IDNum, @IDNum, @SemesterID, @ProgCode, 2026)";
                        ...
                        else
                        {
                            ...
                            string updateQuery;
                            if (role == "Admin")
                                updateQuery = "UPDATE HeadofProgramme SET HopName = @Name, HopEmail = @Email, Password = @Password, IdentityNumber = @IDNum WHERE HopID = @ID";
                            else if (role == "Lecturer")
                                updateQuery = "UPDATE Lecturer SET LecturerName = @Name, LecturerEmail = @Email, Password = @Password, IdentityNumber = @IDNum WHERE LecturerID = @ID";
                            else // Student – include IC
                                updateQuery = "UPDATE Student SET StudentName = @Name, StudentEmail = @Email, Password = @Password, IC = @IDNum, IdentityNumber = @IDNum, SemesterID = @SemesterID, ProgrammeCode = @ProgCode WHERE StudentID = @ID";
                ...
                {
                    // Display the error for debugging – you may log it as well
                    ShowStatus("Database error: " + ex.Message, false);
                    pnlModalContainer.Visible = true;
                }
                catch (Exception ex)
                {
        ...
        protected void gvUsers_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            if (e.CommandName == "EditUser")
            {
                string query;
                if (targetRole == "Admin")
                    query = "SELECT HopName AS Name, HopEmail AS Email, Password, IdentityNumber, NULL as SemesterID, NULL as ProgrammeCode FROM HeadofProgramme WHERE HopID = @ID";
                else if (targetRole == "Lecturer")
                    query = "SELECT LecturerName AS Name, LecturerEmail AS Email, Password, IdentityNumber, NULL as SemesterID, NULL as ProgrammeCode FROM Lecturer WHERE LecturerID = @ID";
                else // Student – retrieve IC (the unique identifier), not IdentityNumber
                    query = "SELECT StudentName AS Name, StudentEmail AS Email, Password, IC, SemesterID, ProgrammeCode FROM Student WHERE StudentID = @ID";
                ...
                using (SqlConnection conn = new SqlConnection(connString))
                {
                    using (SqlCommand cmd = new SqlCommand(query, conn))
                    {
                        ...
                        try
                        {
                            ...
                            using (SqlDataReader reader = cmd.ExecuteReader())
                            {
                                if (reader.Read())
                                {
                                    ...
                                    // For students, the column is named "IC"; for Admin/Lecturer it's "IdentityNumber"
                                    if (targetRole == "Student")
                                        txtIdentityNumber.Text = reader["IC"].ToString();
                                    else
                                    ...
                                    if (targetRole == "Student")
                                    {
                                        ...
                                        if (ddlStudentProgramme.Items.FindByValue(progCode) != null)
                                            ddlStudentProgramme.SelectedValue = progCode;
                                    }
                                    else
                                    {
                        ...
                        {
                            ShowStatus("Error loading user: " + ex.Message, false);
                        }
                    }
                }
            }
            ...
            {
                string deleteQuery = targetRole == "Admin" ? "DELETE FROM HeadofProgramme WHERE HopID = @ID AND HopID <> 1;" :
                                     targetRole == "Lecturer" ? "DELETE FROM Lecturer WHERE LecturerID = @ID;" :
                                     "DELETE FROM Student WHERE StudentID = @ID;";
                using (SqlConnection conn = new SqlConnection(connString))
                {
                    using (SqlCommand cmd = new SqlCommand(deleteQuery, conn))
                    {
                        ...
                        {
                            ShowStatus("Delete failed: " + ex.Message, false);
                        }
                    }
                }
            }
        ...
        private void ResetFormState()
        {
            ...
            ddlStudentProgramme.SelectedIndex = 0;
            pnlStudentFields.Visible = false;
            btnCreateAccount.Text = "Register Account";
